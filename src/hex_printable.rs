...
pub trait HexPrintable {
    ...
    unsafe fn as_hex<'a>(&'a self) -> Result<DynArray<u8>, KernelError> {
        unsafe {
            ...
            let mut buf = KERNEL.mem.with_unwrap(|mem| DynArray::new(byte_count, false, mem));
            self.convert_to_bytes(&mut buf)?;
            let mut hex_chars= KERNEL.mem.with_unwrap(|mem| DynArray::new(byte_count * 2, false, mem));
